...
#define FROM_GATEWAY 0
#define FROM_ENDPOINT 1
...
int test_send(netdev_t *netdev, char* payload, int len){
	printf("sending ");
	for (int k = 0; k < (int)len; ++k) printf("%x",(int)message[k]);
	printf(" (%d bytes)\n",(int)len);
...
int main(void){
    ...
    netopt_enable_t iq_inverted = NETOPT_DISABLE;//ENABLE;
    ...
    int transmissionDirection = FROM_ENDPOINT;
    while(1){
		//char buffer[256]="";
		//char temp[256]="";
		if (flagMessageRecv!=0){
			for (int k = 0; k < (int)len; ++k){
				//memcpy(&buffer[k],message[k],sizeof(uint16_t));
				//if ((int)message[k]!=0){
					printf("%x",(int)message[k]);
					//sprintf(buffer,"%x",(int)message[k]);
					//printf("message : %s\n",message[k]);
			...
			printf(" (%d bytes)\n",(int)len);
			set_param(&sx126x,868100000,125,SPREADING_FACTOR,5);
			//iq_inverted = NETOPT_DISABLE;//ENABLE;
			//syncword = LORA_SYNCWORD_PRIVATE;
			netdev->driver->get(netdev, NETOPT_IQ_INVERT, &mytemp, sizeof(uint8_t));
			printf("iq invert : %i\n",mytemp);
			fflush(stdout);
			netdev->driver->set(netdev, NETOPT_IQ_INVERT, &iq_inverted, sizeof(netopt_enable_t));
			netdev->driver->set(netdev, NETOPT_SYNCWORD, &syncword, sizeof(uint8_t));
			//if (transmissionDirection == FROM_GATEWAY){
				wdt_setup_reboot(0, 8000); // MAX_TIME in ms
				wdt_start();
			//}
			if (test_send(netdev,message,len)<=0){
				printf("fail\n");
			}
			else{
				printf("ok\n");
			}
			//if (transmissionDirection == FROM_ENDPOINT){
				wdt_kick();
			//}
			//syncword = LORA_SYNCWORD_PUBLIC;
			//netdev->driver->set(netdev, NETOPT_SYNCWORD, &syncword, sizeof(uint8_t));
			//set_param(&sx126x,868100000,125,SPREADING_FACTOR,5);
			
			fflush(stdout);
			//temp[0]='\0';
			while(flagTxDone!=1){	//To remove
				wait(22);
				printf("attente\n");
			}
			fflush(stdout);
			flagTxDone=0;
			if (transmissionDirection == FROM_ENDPOINT){
				transmissionDirection=FROM_GATEWAY;
				iq_inverted = NETOPT_ENABLE;
				//syncword = LORA_SYNCWORD_PUBLIC;
			}
			else if (transmissionDirection == FROM_GATEWAY){
				transmissionDirection=FROM_ENDPOINT;
				iq_inverted = NETOPT_DISABLE;
				//syncword = MY_SYNCWORD;
			}
			netdev->driver->set(netdev, NETOPT_IQ_INVERT, &iq_inverted, sizeof(netopt_enable_t));
			netdev->driver->get(netdev, NETOPT_IQ_INVERT, &mytemp, sizeof(uint8_t));
			printf("iq invert : %i\n",mytemp);
			//netdev->driver->set(netdev, NETOPT_SYNCWORD, &syncword, sizeof(uint8_t));
			//netdev->driver->get(netdev, NETOPT_SYNCWORD, &mytemp, sizeof(uint8_t));
			//printf("syncword : %x\n",mytemp);
			flagMessageRecv=0;
		}
		fflush(stdout);
